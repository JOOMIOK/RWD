...
var viewport = window.matchMedia('(max-width:999px)');/* 999px 에서 적용*/
var item = $('.menu-item');
var li = $('.menu-list');
if(viewport.matches){
  //   모바일 디자인에서 실행할 코드 블럭
  ...
  li.addClass('icon-plus'); /*모든  li 에 +추가 */
  item.attr('role','button');
  item.attr('aria-haspopup','true')
  item.attr('aria-pressed', 'false');
  item.attr('aria-expanded', 'false');

  // 버튼을 (btn) 을 클릭하면 네이게이션(nav) 요소를 찾아서
  ...
  // 클릭한 버튼의 부모 요소인 .menu-list 에 menu-act라는 클래스를 추가한다.
  // 클릭한 버튼의 부모 요소의 형제 요소들을 찾아 menu-act 클래스를 삭제한다.
